...
public class InvokeDecompiler {
    private static final String UNKNOWN_VARIABLE_NAME = "unknown";

    public static List<String> diagnose(String className, MethodNode mn) throws AnalyzerException {
        ...
        for (int methodInsn : methodInsnArray) {
            ...
            // (3) 将反编译的结果打印出来
            result.add(String.format("%s.%s(%s)", methodInsnNode.owner, methodInsnNode.name, argList));
        }
        return result;
    }
